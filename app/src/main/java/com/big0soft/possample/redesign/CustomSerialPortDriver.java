...
public class CustomSerialPortDriver implements SerialPortDriver {
    int portNo;
    ...
    CustomSerialPortDriver(Context context, int port) {
    ...
    public int send(byte[] data, int dataLen) {
        try {
            if (data != null && data.length > 0 && data.length >= dataLen && dataLen > 0 && dataLen <= COM_MAX_LEN) {
                if (!this.isPortOpen) {
                    return -4004;
                } else {
    ...
    public int recv(byte[] buffer, int recvLen, long timeout) {
        try {
            if (buffer != null && buffer.length > 0 && recvLen > 0 && timeout >= 0L && buffer.length >= recvLen && recvLen <= COM_MAX_LEN) {
                if (!this.isPortOpen) {
                    return -4004;
                } else {
    ...
    public int recv(byte[] buffer, int recvLen) {
        try {
            if (buffer != null && buffer.length > 0 && buffer.length >= recvLen && recvLen > 0 && recvLen <= COM_MAX_LEN) {
                if (!this.isPortOpen) {
                    return -4004;
                } else {
